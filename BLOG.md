...
**flat at 0.40–0.46** — the *frozen subagent doesn't learn*. On
...
The delegation dynamics reproduce cleanly — that is the point of this
...
`scripts/run-qwen3-4B-parl.sh` → **Delegate-only** (swarm-strict),
`scripts/run-qwen3-4B-orchestrator_only.sh` → **PARL** (swarm-paper).
