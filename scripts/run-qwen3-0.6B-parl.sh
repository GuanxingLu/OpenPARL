...
# Thin wrapper around openparl/openparl.run with --env widesearch.
#
# Prereq: local RAG server running on :8000. Launch it in a separate tmux:
#   bash openparl/widesearch/launch_rag_server.sh
# and verify:
...
export OPENPARL_SUBAGENT_MAX_TURNS=${OPENPARL_SUBAGENT_MAX_TURNS:-4}
export OPENPARL_SUBAGENT_MAX_TOOLCALLS=${OPENPARL_SUBAGENT_MAX_TOOLCALLS:-6}
...
if [ "$SUBAGENT_MODE" = "frozen" ]; then
   SGLANG_EXTRA_ARGS=(--sglang-config openparl/sglang_config_0.6B.yaml)
