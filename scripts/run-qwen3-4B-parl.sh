...
# PARL widesearch run: paper-faithful swarm (K2.5 Appendix E.8).
#
# Orchestrator gets direct search/browse/python in addition to
# create_subagent / assign_task. Frozen-subagent topology and everything
# else matches run-qwen3-4B-delegate-only.sh; only --agent-mode and the
# save-subdir differ. Prereq: local RAG server running on :8000.
